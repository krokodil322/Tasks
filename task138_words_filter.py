...
words = sorted(filter(lambda word: True if len(word) == 6 else False, words))
print(*words)
...
# numbers = map(lambda num: num // 2, filter(lambda num: True if (num > 47) and (num % 2 == 0) else False, numbers))
numbers = map(lambda num: num // 2 if (num % 2 == 0) else num, filter(lambda num: False if (num % 2 != 0) and (num > 47) else True, numbers))
